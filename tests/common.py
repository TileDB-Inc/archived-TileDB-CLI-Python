...
test_array_names = [
    "dense_25x12",
    "dense_25x12x3",
    "dense_25x12_mult_attr",
    "sparse_25x12",
    "sparse_25x12_mult_attr",
]
...
def create_test_array_dense_25x12x3(temp_rootdir):
    ...
    with tiledb.DenseArray(path, mode="w") as A:
        data = np.arange(900)
        np.reshape(data, (25, 12, 3))
        A[:] = data


@pytest.fixture(autouse=True, scope="session")
def create_test_array_dense_25x12_mult_attr(temp_rootdir):
    """
    Create a simple dense test array.
    """
    path = os.path.abspath(os.path.join(temp_rootdir, "dense_25x12_mult_attr"))

    ctx = tiledb.default_ctx()
    rows_dim = tiledb.Dim(ctx=ctx, domain=(1, 25), dtype=np.int64)
    cols_dim = tiledb.Dim(ctx=ctx, domain=(1, 12), dtype=np.int64)
    dom = tiledb.Domain(rows_dim, cols_dim, ctx=ctx)
    att1 = tiledb.Attr(name="a", ctx=ctx, dtype=np.int64)
    att2 = tiledb.Attr(name="b", ctx=ctx, dtype=np.int64)
...
@pytest.fixture(autouse=True, scope="session")
def create_test_array_sparse_25x12_mult_attr(temp_rootdir):
    """
    Create a simple sparse test array.
    """
    path = os.path.abspath(os.path.join(temp_rootdir, "sparse_25x12_mult_attr"))
