...
from dump import tiledb_top_level_entry
...
def test_dump_config():
    ...
    """
    runner = CliRunner()
    result = runner.invoke(tiledb_top_level_entry, ["dump", "config"])
...
def test_dump_metadata(temp_rootdir, array_name, pp):
    ...
    runner = CliRunner()
    result = runner.invoke(tiledb_top_level_entry, ["dump", "metadata", uri])
    assert result.exit_code == 0
...
def test_dump_nonempty_domain(temp_rootdir, array_name, pp):
    ...
    runner = CliRunner()
    result = runner.invoke(tiledb_top_level_entry, ["dump", "nonempty-domain", uri])
    assert result.exit_code == 0
...
def test_dump_schema(temp_rootdir, array_name, pp):
    ...
    runner = CliRunner()
    result = runner.invoke(tiledb_top_level_entry, ["dump", "schema", uri])
    assert result.exit_code == 0
...
@pytest.mark.skip(reason="Incomplete dump array command")
@pytest.mark.parametrize("array_name", test_array_names)
def test_dump_array(temp_rootdir, array_name):
    ...
    runner = CliRunner()
    result = runner.invoke(tiledb_top_level_entry, ["dump", "array", uri])
    assert result.exit_code == 0
